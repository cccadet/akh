...
function App() {
  ...
  return <div className="shell">
    ...
    <main>
      ...
      <section id="tasks"><div className="section-title"><h2>Tasks</h2><span>{state.tasks.length}</span></div><div className="tasks">{state.tasks.map((task) => <article key={task.id}><div className="task-id">#{task.id}</div><div className="task-copy"><h3>{task.title}</h3><p>{task.project} · {task.branch} · {task.lastAgent ?? "no agent yet"}</p><small>{task.synced ? "Shared" : "Local"} · {task.latestCommit?.slice(0,10) ?? "Not started"}</small></div><div className="actions"><button onClick={() => handoff(task.id)}>Handoff</button><button onClick={() => launch(task.id,"claude")}>Claude</button><button className="primary" onClick={() => launch(task.id,"codex")}>Codex</button></div></article>)}</div></section>
      <section id="settings"><div className="section-title"><h2>Settings</h2></div><div className="settings"><label>Worktrees<strong>{state.worktreeRoot}</strong></label><label>Terminal<strong>{state.terminalCommand}</strong></label></div></section>
    </main>
  </div>;
}
